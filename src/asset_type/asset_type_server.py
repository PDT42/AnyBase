...
from asset_type.bookable_manager_extension import BookableManager
...
class AssetTypeServer:
    ...
    async def post_create_asset_type():
        ...
        # Getting the super type if defined.
        if super_type_id:
            super_type = asset_type_manager.get_one_by_id(super_type_id)

        # Initialize the new AssetType.

        new_asset_type: AssetType = AssetType(
            asset_name=asset_name,
            columns=columns,
            super_type=super_type_id,
            owner_id=owner_id)

        new_asset_type = asset_type_manager.create_asset_type(new_asset_type)

        if bookable_type:
            bookable_manager = BookableManager()
            bookable_manager.init_bookable_type(new_asset_type)
    ...
    async def get_create_overview_page_layout(asset_type: AssetType):
        ...
        available_plugins: List[Plugin] = list(filter(
            lambda plugin: plugin.plugin_type in [PluginType.ASSET_TYPE, PluginType.HYBRID],
