...
class AssetTypeManager(AAssetTypeManager):
    ...
    def create_asset_type(self, asset_type: AssetType) -> AssetType:
        ...
        # Making sure, that if this is a subtype, it has a super_type
        if asset_type.is_subtype and not asset_type.super_type_id:
            raise IllegalStateException(
                "If an asset type is_subtype, it needs another asset type to be sub to!")
        ...
        if asset_type.is_subtype:
            ...
            asset_type.columns.append(Column(
                column_name, column_name, DataTypes.INTEGER.value, required=True)
            )
    ...
    def _convert_result_to_asset_types(results):
        ...
        for asset_type_row in results:
            assets_types.append(AssetType(
                ...
                is_subtype=bool(asset_type_row['is_subtype']),
    ...
    def get_one(self, asset_type_id: int) -> Optional[AssetType]:
        ...
        asset_type = AssetType(
            ...
            is_subtype=bool(result[0]['is_subtype']),
            super_type_id=int(result[0]['super_type_id'])
        )
