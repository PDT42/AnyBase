...
function createPlugin(plugin, root_id, column_info) {
    ...
    let assetType = asset_type_root['asset-type'];
    let asset = asset_type_root['asset'];
    if (asset !== undefined) {
        let assetName = generatePluginAssetName(
            plugin['id'], assetType['asset-type-id'], asset['asset_id'])
    } else {
        let assetName = generatePluginAssetName(
            'basic-notes', assetType['asset-type-id'])
    }
