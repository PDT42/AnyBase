...
class AssetTypeManager(AAssetTypeManager):
    ...
    def update_asset_type(self, asset_type: AssetType, extend_columns: bool = True) -> AssetType:
        ...
        if len(db_asset_type.columns) != len(asset_type.columns):
            ...
            # TODO: implement remove, append columns - This must
            # TODO: include checking whether the "more" columns
            # TODO: of the parameter supplied AT are columns of
            # TODO: the super_type.
    ...
    def _convert_result_to_asset_type(self, result: Mapping) -> AssetType:
        ...
        return AssetType(
            asset_name=str(result[self.ASSET_NAME]),
            ...
            asset_table_name=str(result.get('asset_table_name', None)),
            asset_type_id=int(result[self.PRIMARY_KEY]),
            super_type=int(result['super_type']),
            is_slave=bool(result['is_slave']),
            owner_id=int(result['owner_id']),
            bookable=bool(result['booking_type_id'] > 0),
