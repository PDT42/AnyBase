...
function createNoteInput(column_info) {
    ...
    let create_route = column_info['sources']['create-plugin-asset'];
...
function createPlugin(plugin, root_id, column_info) {

    let asset_type_root = document.getElementById(root_id);
    let asset_type = asset_type_root['asset-type'];
    let asset = asset_type_root['asset'];

    let plugin_asset_name = null;
    if (asset !== undefined) {
        plugin_asset_name = generatePluginAssetName(
            plugin['id'], asset_type['asset-type-id'], asset['asset_id'])
    } else {
        plugin_asset_name = generatePluginAssetName(
            'basic-notes', asset_type['asset-type-id'])
    }
    ...
    root['stream_data'].forEach((obs_info, channel) => {
        ...
        if (channel.startsWith('stream-' + plugin_asset_name)) {
